...
def test_fetch_gtdb_genome_ids(setup_and_teardown, mocker):
    tmp_dir, script_file = setup_and_teardown

    mock_form_url = mocker.patch('src.loaders.ncbi_downloader.ncbi_downloader._form_gtdb_taxonomy_file_url')
    mock_form_url.return_value = ['https://data.gtdb.ecogenomic.org/releases/release202/202.0/ar122_taxonomy_r202.tsv']

    release_ver = '202'
    genome_ids = ncbi_downloader._fetch_gtdb_genome_ids(release_ver, tmp_dir)
    assert len(genome_ids) == 4316
