...
Note: If the ArangoDB collection has been previously created using a JSON file generated by the genome attributes
      loader script, it is crucial to ensure that the arguments "--load_ver" and "--kbase_collection" are consistent
      with the ones used in the genome attributes loader script in order to ensure that the same key is generated
      for the corresponding Arango document."
...
def _create_doc(row, kbase_collection, load_version, genome_id, features, pre_fix='KBase'):
    ...
    if features:
        doc.update(row[list(features)].rename(lambda x: pre_fix + '_' + x).to_dict())
    else:
        doc.update(row.rename(lambda x: pre_fix + '_' + x).to_dict())

    return doc


def _row_to_doc(row, kbase_collection, load_version, features, tool_genome_map, genome_id_col, pre_fix):
    ...
    doc = _create_doc(row, kbase_collection, load_version, genome_id, features, pre_fix=pre_fix)
...
def _read_tool_result(result_dir, batch_dir, kbase_collection, load_ver, tool_file_name, features, genome_id_col,
    ...
    if os.path.exists(tool_file):
        ...
        docs = df.apply(_row_to_doc, args=(kbase_collection, load_ver, features, tool_genome_map,
                                           genome_id_col, pre_fix), axis=1).to_list()
...
def gtdb_tk(root_dir, kbase_collection, load_ver):
    ...
    genome_id_col, pre_fix = 'user_genome', 'gtdb_tk'
...
def checkm2(root_dir, kbase_collection, load_ver):
    ...
    tool_file_name, genome_id_col, pre_fix = 'quality_report.tsv', 'Name', 'checkm2'
    for batch_dir in batch_dirs:
        docs = _read_tool_result(result_dir, batch_dir, kbase_collection, load_ver,
