...
def _create_doc(row, kbase_collection, load_version, genome_id, features, pre_fix):
    ...
    if features:
        doc.update(row[list(features)].rename(lambda x: pre_fix + '_' + x if pre_fix else x).to_dict())
    else:
        doc.update(row.rename(lambda x: pre_fix + '_' + x if pre_fix else x).to_dict())
