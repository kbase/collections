...
Note: The database was previously created using a JSON file generated by the genome attributes loader script
      (e.g. gtdb_genome_attribs_loader.py script).

      It is important to ensure that the arguments "--load_ver" and "--kbase_collection" match the ones used in the
      genome attributes loader script in order to generate the same key for the corresponding Arango document.

usage: parse_computed_genome_attribs.py [-h] --load_ver LOAD_VER [--tools TOOLS [TOOLS ...]]
